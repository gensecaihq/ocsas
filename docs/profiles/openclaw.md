...
**Node Execution Control (`system.run`):**
If a macOS node is paired, the Gateway can invoke `system.run` on that node (remote code execution):
...
`/reasoning` and `/verbose` can expose internal reasoning or tool output not meant for public channels:
